    ...
    : ControllerBase
{
    ...
    public async Task<ActionResult<Challenge>> GenerateNewChallenge(int languageId, [FromQuery] Difficulty difficulty)
    {
        try
        {
            ...
            var challenge = await challengeService.GenerateUserChallenge(userId, languageId, difficulty);
            return Ok(challenge);
        }
        ...
        [FromQuery] int pageSize = 10)
    {
        ...
        {
            var userId = User.GetUserId();
            var challenges = await challengeService.GetCompletedChallenges(userId, languageId, page, pageSize);
        ...
        [FromQuery] int pageSize = 10)
    {
        ...
        {
            var userId = User.GetUserId();
            var challenges = await challengeService.GetAttemptingChallenges(userId, languageId, page, pageSize);
            return Ok(challenges);
        }
        ...
        [FromBody] ChallengeUpdateDto updateDto)
    {
        ...
        {
            var userId = User.GetUserId();
            await challengeService.UpdateUserProgress(userId, challengeId, updateDto.IsCorrect);
            return Ok();
        }
